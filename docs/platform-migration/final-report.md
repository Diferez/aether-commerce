...
- `agent-core`: shared intent list, deterministic execution planning, mutable-tool authorization, PII redaction, composable Gemini prompts, provider-neutral model execution, conversation-memory ownership and tool-telemetry policy; the Worker remains the Cloudflare/Gemini/D1 adapter.
...
- assistant intent, provider runtime, memory lifecycle, execution planning and audit/counter ordering -> `packages/agent-core/src/`; D1 persistence remains in `apps/ai-assistant/adapters/`.
...
- On the current validation checkpoint, `pnpm typecheck`, `pnpm lint`, `pnpm test` (10 tests), `pnpm test:unit` (51 tests), `pnpm openapi:check`, `pnpm validate`, `pnpm build`, `pnpm check:boundaries`, `pnpm test:client-template`, `pnpm test:e2e` and `pnpm test:e2e:assistant` all pass.
...
- `agent-core` still needs reusable concrete tool execution; execution planning, conversation-memory lifecycle and tool telemetry/counter ordering now live in the package, with D1 encoding in the Worker adapter.
