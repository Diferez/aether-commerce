...
- Remaining D1-bound work is explicit: admin product overrides, settings/audit operations and public shipping reads. Customer profile, addresses, customer-scoped order reads, public/customer/admin-review operations, coupon and contact persistence, inventory, order-status administration and idempotent webhook event persistence now have reusable ports. Provider signature verification remains intentionally in the integration adapter.
