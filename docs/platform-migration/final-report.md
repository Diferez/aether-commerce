...
- `api-core`: pure cart/catalog/order and contact-message operations, customer profile, preferences, addresses, customer-scoped order reads and reviews, inventory operations and a provider-neutral checkout port; `apps/api` remains the D1/Stripe adapter.
...
- On commit `e01afce`, `pnpm install --frozen-lockfile`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (10 tests), `pnpm test:unit` (38 tests), `pnpm openapi:check`, `pnpm validate` and `pnpm build` all pass on the migration branch.
...
- Remaining D1-bound work is explicit: admin product overrides, coupon/review/settings/audit operations and public shipping reads. Customer profile, addresses, customer-scoped order reads and reviews, contact persistence, inventory, order-status administration and idempotent webhook event persistence now have reusable ports. Provider signature verification remains intentionally in the integration adapter.
