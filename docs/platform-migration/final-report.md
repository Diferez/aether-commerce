...
- `api-core`: pure cart/catalog/order operations, customer preferences, addresses and customer-scoped order reads, inventory operations and a provider-neutral checkout port; `apps/api` remains the D1/Stripe adapter.
...
- Remaining D1-bound work is explicit: user profile/review persistence; admin product overrides, coupon/review/settings/audit operations; public shipping reads; and contact persistence. Customer addresses, customer-scoped order reads, inventory, order-status administration and idempotent webhook event persistence now have reusable ports. Provider signature verification remains intentionally in the integration adapter.
