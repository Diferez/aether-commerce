...
if (production && process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY && !process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY.startsWith("pk_live_")) {
  console.error("Production requires a Clerk pk_live_ publishable key.");
  process.exitCode = 1;
}

if (production && process.env.CLERK_SECRET_KEY && !process.env.CLERK_SECRET_KEY.startsWith("sk_live_")) {
  console.error("Production requires a Clerk sk_live_ secret key.");
  process.exitCode = 1;
}
